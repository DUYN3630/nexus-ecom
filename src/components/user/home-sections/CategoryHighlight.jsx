          ...
          {categories.map((cat, index) => {
            ...
            return (
              <div 
                ...
                className={`group relative h-[400px] bg-slate-900 rounded-3xl overflow-hidden cursor-pointer transition-all duration-700 hover:shadow-2xl hover:shadow-indigo-100`}
              >
                {/* Image background */}
                <div className="absolute inset-0 w-full h-full transition-transform duration-1000 group-hover:scale-110">
                  <img 
                    ...
                    loading="lazy"
                  />
                  {/* Overlay to ensure text readability - Always dark for white text */}
                ...
                <div className="absolute inset-0 p-10 flex flex-col justify-end z-10">
                  <div className="space-y-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <h3 className={`text-3xl font-black uppercase tracking-tighter italic text-white drop-shadow-lg`}>
                      {cat.name}
                    </h3>
                    {cat.description && (
                        <p className={`text-[11px] font-bold uppercase tracking-widest text-white opacity-90 max-w-[200px] leading-relaxed line-clamp-2`}>
                        {cat.description}
                        </p>
                    )}
                    <div className="pt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                      <span className={`inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-white bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20`}>
