...
import {
  ...
  UserCog, History, ShieldCheck, Home, Moon, Sun, Settings, User, Ticket
} from 'lucide-react';
...
export const AdminSidebar = () => {
  ...
  // Fix hiển thị role chuyên nghiệp hơn
  ...
  const menuGroups = [
    {
      ...
      items: [
        ...
        { name: 'Đơn hàng', icon: ShoppingCart, path: '/admin/orders', badge: '12' },
        { name: 'Yêu cầu hỗ trợ', icon: Ticket, path: '/admin/tickets' },
  ...
  return (
    <aside className="w-72 bg-white border-r border-gray-200 flex flex-col h-full hidden lg:flex relative">
      ...
      {/* Footer Sidebar with Drop-up Account Menu */}
      <div className="p-4 border-t border-gray-100 bg-white relative" ref={dropdownRef}>
        
        {/* Drop-up Menu */}
        {isProfileOpen && (
          <div className="absolute bottom-[calc(100%-8px)] left-4 right-4 bg-white border border-slate-200 rounded-2xl shadow-2xl p-2 mb-2 animate-in slide-in-from-bottom-2 duration-200 z-50 overflow-hidden">
            ...
            {/* Group 2: Hệ thống */}
            <div className="p-2">
               <p className="px-2 pb-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hệ thống</p>
               <button className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-lg transition-colors text-left">
            ...
            {/* Group 3: Tiện ích */}
            <div className="p-2">
              <div className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => setIsDarkMode(!isDarkMode)}>
                <div className="flex items-center gap-3">
                  {isDarkMode ? <Moon className="w-4 h-4 text-indigo-500" /> : <Sun className="w-4 h-4 text-amber-500" />}
                  <span className="text-sm text-slate-600">Chế độ tối</span>
                </div>
                <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${isDarkMode ? 'bg-brand-600' : 'bg-slate-200'}`}>
              ...
              <button 
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-rose-500 hover:bg-rose-50 rounded-lg transition-colors text-left mt-2"
              >
        ...
        {/* Account Trigger Area */}
        <button 
          onClick={() => setIsProfileOpen(!isProfileOpen)}
          className={`w-full flex items-center gap-3 p-2 rounded-2xl transition-all duration-200 border-2 ${
            isProfileOpen ? 'bg-slate-50 border-brand-100' : 'bg-white border-transparent hover:bg-slate-50'
          }`}
        >
