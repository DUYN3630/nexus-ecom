...
| 8 | Thiếu tính năng Expert tự nhận Ticket | Trước đây chỉ có Admin mới có quyền gán Expert cho Ticket. | Bổ sung API `claimTicket` và `sendExpertMessage` trong `ticketController.js` để Expert chủ động nhận việc và chat với khách. | ✅ Thành công |
| 9 | 404 Not Found (Repair Tracking) | Xung đột route hoặc shadowing do mount `/api/support` và định nghĩa route con chưa chuẩn. | Đổi tên endpoint thành `/repair-tracking/:phone`, viết lại `supportRoutes.js` sạch và dọn dẹp `index.js`. | ✅ Thành công |
| 10 | 403 Forbidden (Customer Confirm Repair) | API cập nhật trạng thái yêu cầu quyền `adminOrExpert`, chặn đứng Khách hàng khi họ muốn duyệt báo giá. | Thêm route public `/repair/:id/confirm` và phương thức `confirmRepair` trong controller với bảo mật dựa trên SĐT. | ✅ Thành công |
| 11 | Mất dữ liệu Phí dịch vụ (Service Fee) | Model `RepairRequest` thiếu trường `serviceFee` nên không lưu được vào DB. | Bổ sung `serviceFee` vào Schema và cập nhật logic `updateStatus` trong controller. | ✅ Thành công |
| 12 | Linh kiện (usedParts) không hiển thị chi tiết | API trả về danh sách sửa chữa nhưng không `populate` thông tin linh kiện. | Thêm `.populate('usedParts.part')` vào tất cả các hàm query trong `repairController.js`. | ✅ Thành công |
